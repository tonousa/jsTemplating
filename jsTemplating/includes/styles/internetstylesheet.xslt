...
<xsl:variable name="global_font_size">-1</xsl:variable>
...
<!-- *** choose separation bar: 'red', 'line', 'nothing' *** -->
<xsl:variable name="choose_sep_bar">red</xsl:variable>
...
<xsl:variable name="res_snippet_size">90%</xsl:variable>
...
<xsl:variable name="faint_color2">#008000</xsl:variable>
...
<xsl:variable name="choose_adv_search_page_header">provided</xsl:variable>
...
<xsl:variable name="adv_search_panel_bgcolor">#a3a3a3</xsl:variable> 
...
<xsl:variable name="sep_bar_bg_color">
  <xsl:choose>
    <xsl:when test="$choose_sep_bar = 'red'">#cc0000</xsl:when>
...
<xsl:variable name="sep_bar_text_color">
  <xsl:choose>
    <xsl:when test="$choose_sep_bar = 'red'">#ffffff</xsl:when>
...
.l{font-color: <xsl:value-of select="$res_title_color"/>}
...
.g,.g:link,.g a:link{color:<xsl:value-of select="$faint_color2"/>}
...
.b{font-size: 11px; color:#0000cc; font-weight:bold}
...
<!-- *** nav_url: does not include q, num and start elements *** -->
...
<!-- *** search_url: $nav_url + num and start elements *** -->
<xsl:variable name="search_url">
  <xsl:value-of select="$nav_url"/>&amp;num=<xsl:value-of
    select="/GSP/PARAM[@name='num']/@original_value"/>&amp;start=<xsl:value-of
    select="/GSP/PARAM[@name='start']/@original_value"/>
</xsl:variable>

<!-- *** adv_search_url: /search? + $search_url + as_q=$q *** -->
<xsl:variable name="adv_search_url">/search?<xsl:value-of 
  select="$search_url"/>&amp;as_q=<xsl:value-of 
  select="/GSP/PARAM[@name='q']/@original_value"
...
<!-- *** url_escaped_query *** -->
<xsl:variable name="url_escaped_query"
  select="GSP/PARAM[@name='q']/@original_value"/>

<!-- *** stripped_search_query: q, as_q, ... *** -->
...
<!-- <xsl:variable name="cached_page_html" select="CACHE/CACHE_HTML"/> -->
  ...
  <xsl:variable name="cached_page_url">
   <xsl:call-template name="replace_string">
    <xsl:with-param name="find" select="'noarchive/'"/>
    <xsl:with-param name="replace" select="''"/>
    <xsl:with-param name="string" select="CACHE/CACHE_URL"/>
   </xsl:call-template>  
  </xsl:variable>

  <xsl:variable name="cached_page_html">
   <xsl:call-template name="replace_string">
    <xsl:with-param name="find" select="'noarchive'"/>
    <xsl:with-param name="replace" select="''"/>
    <xsl:with-param name="string" select="CACHE/CACHE_HTML"/>
   </xsl:call-template>  
  </xsl:variable>
...
// </xsl:comment>
  ...
  <!-- <body class="d" onload="setFocus()"> -->
    ...
    <form method="get" action="/search" name="f">
      ...
      <xsl:if test="PARAM[@name='restrict']">
                      ...
                      <td width="85%">
                        ...
                        border="0" cellspacing="0">
                          ...
                          <tr>
                            ...
                            <td>
                              <xsl:choose>
                                <xsl:when test="PARAM[@name='as_epq']">
                                  <input type="text" size="25" 
                                  value="{PARAM[@name='as_epq']/@value}" name="as_epq" />
                                </xsl:when>
                                <xsl:otherwise>
                                  <input type="text" size="25" value="" name="as_epq" />
                                </xsl:otherwise>
                              </xsl:choose>
                            </td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td>
                              <xsl:choose>
                                <xsl:when test="PARAM[@name='as_oq']">
                                  <input type="text" size="25" 
                                  value="{PARAM[@name='as_oq']/@value}" name="as_oq" />
                                </xsl:when>
                                <xsl:otherwise>
                                  <input type="text" size="25" value="" name="as_oq" />
                                </xsl:otherwise>
                              </xsl:choose>
                            </td>
                          </tr>
                          ...
                          <tr>
                            ...
                            <td>
                              <xsl:choose>
                                <xsl:when test="PARAM[@name='as_eq']">
                                  <input type="text" size="25" 
                                  value="{PARAM[@name='as_eq']/@value}" name="as_eq" />
                                </xsl:when>
                                <xsl:otherwise>
                                  <input type="text" size="25" value="" name="as_eq" />
                                </xsl:otherwise>
                              </xsl:choose>
                            </td>
                          </tr>
...
<!--
                        <xsl:choose>
                          <xsl:when test="PARAM[(@name='lr') and (@value='lang_ar')]">
                            <option value="lang_ar" selected="selected">Arabic</option>
                          </xsl:when>
                          <xsl:otherwise>
                        ...
                        <xsl:choose>
                          <xsl:when test="PARAM[(@name='lr') and (@value='lang_cz')]">
                            <option value="lang_cz" selected="selected">Czech</option>
                          </xsl:when>
                          <xsl:otherwise>
                            <option value="lang_cz">Czech</option>
                          </xsl:otherwise>
                        </xsl:choose>
            ...
            <td>
              <xsl:choose>
                <xsl:when test="$access='p'">
                  <input type="radio" name="access" value="p" checked="checked" />public
                </xsl:when>
                <xsl:otherwise>
                  <input type="radio" name="access" value="p"/>public
                </xsl:otherwise>
              </xsl:choose>
                <xsl:choose>
                  <xsl:when test="$access='s'">
                    <input type="radio" name="access" value="s" checked="checked" />secure
                  </xsl:when>
                  <xsl:otherwise>
                    <input type="radio" name="access" value="s"/>secure
                  </xsl:otherwise>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="$access='a'">
                  <input type="radio" name="access" value="a" checked="checked" />all
                </xsl:when>
                <xsl:otherwise>
                  <input type="radio" name="access" value="a"/>all
                </xsl:otherwise>
              </xsl:choose>
            </td>
          </tr>
...
<xsl:template name="sort_by">
  <xsl:variable name="sort_by_relevance_url"><xsl:for-each 
    select="/GSP/PARAM[(@name != 'sort') and (@name != 'ip')]">
      <xsl:value-of select="@name"/><xsl:text>=</xsl:text>
      <xsl:value-of select="@original_value"/>
      <xsl:if test="position() != last()">
        <xsl:text disable-output-escaping="yes">&amp;</xsl:text>
  ...
  <xsl:variable name="sort_by_date_url"><xsl:for-each 
    select="/GSP/PARAM[(@name != 'ip')]">
      <xsl:value-of select="@name"/><xsl:text>=</xsl:text>
      <xsl:value-of select="@original_value"/>
      <xsl:text disable-output-escaping="yes">&amp;</xsl:text>
    </xsl:for-each>sort=date%3AD%3AS%3Ad1</xsl:variable>
...
<xsl:template name="results">
    ...
    <xsl:if test="$show_top_navigation != '0'">
      <td align="left">
        <xsl:call-template name="google_navigation">
          <xsl:with-param name="query" select="$nav_url"/>
  ...
  <!-- *** Handle stopwords suggestions, if any *** -->
  ...
  <!-- *** Filter note (if needed) *** -->
    <xsl:if test="(RES/FI) and (not(RES/NB/NU))">
      <p>
        ...
        already displayed.
        <br/>If you like, you can <a 
          href="/search?q={$url_escaped_query}&amp;filter=0&amp;{$search_url}">
...
<xsl:template name="stopwords">
  <xsl:variable name="stopwords_suggestions">
    <xsl:call-template name="replace_string">
      ...
      <xsl:with-param name="replace" select="'/search/basics.asp#stopwords'"/>
  ...
  <xsl:if test="/GSP/CT">
    <p><xsl:value-of disable-output-escaping="yes" 
         select="$stopwords_suggestions"/></p>
  </xsl:if>
...
<xsl:template name="spelling">
  <xsl:if test="/GSP/Spelling/Suggestion">
    <p><span class="p"><font color="{$spelling_text_color}">
       ...
       <a href="/search?q={/GSP/Spelling/Suggestion[1]/@q}&amp;spell=1&amp;{$search_url}">
       <xsl:value-of disable-output-escaping="yes" 
...
<xsl:template match="R">
  ...
  <xsl:if test="$show_res_title != '0'"> 
    ...
    <xsl:choose>
     ...
     <xsl:when 
      ...
      <xsl:when test="@MIME='application/ms-word'">[MS WORD]</xsl:when>
      <xsl:otherwise>
        [
 ...
 -->
    <a href="/search?type=cache&amp;q=cache:{$docid}{$full_url}+{$stripped_search_query}&amp;{$base_url}">

<!-- Here we use the title contained in the meta-tag for display -->   
    <span class="l">
    <xsl:choose>
      <xsl:when test="T">
		<xsl:choose>
		 <xsl:when test="MT/@N = 'title' and MT[@N = 'title']/@V != ''">
		  <xsl:value-of disable-output-escaping='yes' select="MT[@N = 'title']/@V"/>
		 </xsl:when>
		 <xsl:when test="T">
...
<!--  
 ...
 --> 
  ...
  <!-- *** URL *** -->
    ...
    <xsl:if test="not(starts-with($stripped_url, 'noindex!/'))"> 
      <xsl:text disable-output-escaping='yes'>&lt;a class="g" href="</xsl:text
      ><xsl:value-of disable-output-escaping='yes' select="U"
      /><xsl:text disable-output-escaping='yes'>"&gt;</xsl:text>
     </xsl:if> 
  ...
  <!-- *** Link to more links from this site *** -->
      <xsl:if test="HN">
        <br/>
        [
        <a class="f" href="/search?q=site:{$crowded_url}+{
          $url_escaped_query}&amp;{
...
<xsl:template match="C">
    ...
    <xsl:if test="$show_res_cache != '0'">
      <font size="-1">
        ...
        <a class="f" href="/search?type=cache&amp;q=cache:{$docid}{$full_url}+{
...
<xsl:template name="google_navigation">
  ...
  <!-- *** Show previous navigation, if available *** -->
        <xsl:choose>
          <xsl:when test="$prev">
            <td>
              <a href="/search?q={$url_escaped_query}&amp;start={$view_begin -
                      $num_results - 1}&amp;{$nav_url}&amp;num={$num_results}">
  ...
  <!-- *** Google result set navigation *** -->
        ...
        <xsl:call-template name="result_nav">
          <xsl:with-param name="query" select="$url_escaped_query"/>
        ...
        <xsl:choose>
          <xsl:when test="$next">
	        ...
	        <xsl:if test="$navigation_style = 'google'">
                ...
                <span class="b"><xsl:text>Next</xsl:text>
                <xsl:if test="$navigation_style = 'top'">
                  <xsl:text>&gt;</xsl:text>
                </xsl:if>
                </span>
...
<xsl:template name="result_nav">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <td>
        <a href="/search?q={$query}&amp;start={$start}&amp;num={$num_results}&amp;{$nav_url}">
        <xsl:if test="$navigation_style = 'google'">
...
<!-- **********************************************************************
 Utility templates for converting q, as_q, etc. values for display 
     (do not customize)
     ********************************************************************** -->
<xsl:template match="PARAM[@name='as_q']">
  <xsl:value-of select="@value" disable-output-escaping="yes"/>
  <xsl:text> </xsl:text>
</xsl:template>

<xsl:template match="PARAM[@name='as_epq']">
...
<xsl:template match="PARAM[@name='as_oq']">
  <xsl:call-template name="replace_string">
...
<xsl:template match="PARAM[@name='as_eq']">
  <xsl:text>-</xsl:text>
  <xsl:call-template name="replace_string">
    <xsl:with-param name="find" select="' '"/>
    <xsl:with-param name="replace" select="' -'"/>
...
<xsl:template match="PARAM[@name='as_occt' and @value!='any']">
  <xsl:choose>
    <xsl:when test="@value='title'">
      <xsl:text>allintitle:</xsl:text>
    </xsl:when>
    <xsl:when test="@value='body'">
      <xsl:text>allintext:</xsl:text>
    </xsl:when>
    <xsl:when test="@value='url'">
      <xsl:text>allinurl:</xsl:text>
    </xsl:when>
    <xsl:when test="@value='links'">
      <xsl:text>allinlinks:</xsl:text>
    </xsl:when>
  </xsl:choose>
  <xsl:text> </xsl:text>
</xsl:template>

<xsl:template match="PARAM[@name='as_sitesearch']">
  <xsl:if test="/GSP/PARAM[@name='as_dt' and @value='e']">
    <xsl:text>-</xsl:text>
  </xsl:if>
...
<xsl:template match="PARAM[@name='as_lq']">
  <xsl:text>link:</xsl:text>
  <xsl:value-of select="@value"/>
  <xsl:text> </xsl:text>
</xsl:template>

<xsl:template match="PARAM[@name='q']">
  <xsl:value-of select="@value"/>
  <xsl:text> </xsl:text>
</xsl:template>
...
<xsl:template name="copyright">
  <center>
    <br/><br/>
    <p>
    <font face="arial,sans-serif" size="-1" color="#6f6f6f"><xsl:text 
     disable-output-escaping="yes">&amp;copy;</xsl:text>2002 Google</font>
    </p>
  </center>
</xsl:template>
